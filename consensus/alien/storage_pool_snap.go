...
func (snap *Snapshot) initSpData(number uint64) {
	...
	snap.addCheckpointsPool(number)
...
func (snap *Snapshot) addCheckpointsPool(number uint64) {
	checkStorage := make(map[common.Address]*big.Int, 0)
	checkStorage[common.HexToAddress("ux2ecb939dbd005c46b396534b8092c08bd5366bfc")]=new(big.Int).SetUint64(uint64(3963841423605760))
	checkStorage[common.HexToAddress("ux30a90a494d0d69709205375a02e2d6f0caed2596")]=new(big.Int).SetUint64(uint64(1207358256578560))
	checkStorage[common.HexToAddress("ux447286d98430736f5c4e1e2f23a782e6d8da5daf")]=new(big.Int).SetUint64(uint64(706828136611840))
	checkStorage[common.HexToAddress("ux6fddd987761861fd5e380a1b57f803ce49e5c06e")]=new(big.Int).SetUint64(uint64(5293600879411200))
	checkStorage[common.HexToAddress("ux8babcec819d5912cd8320386989fb88e0317026b")]=new(big.Int).SetUint64(uint64(27602761600204800))
	checkStorage[common.HexToAddress("ux92b4f5f034063e83e34f834638ec55aab24d55e7")]=new(big.Int).SetUint64(uint64(479737109544960))
	checkStorage[common.HexToAddress("uxa5ce9cd2f067ba457b866a8eedf93dea89ef9fb4")]=new(big.Int).SetUint64(uint64(6837743627796480))
	checkStorage[common.HexToAddress("uxb6ee552c2949f125e3c82bf1fae1777f7708292a")]=new(big.Int).SetUint64(uint64(24883252732887000))
	checkStorage[common.HexToAddress("uxbc47bf1bb318e9e312be54f14350115c048dd61d")]=new(big.Int).SetUint64(uint64(756273947607040))
	checkStorage[common.HexToAddress("uxe5c4c02c80a65d3702b36bc0e6ba5097404ef50a")]=new(big.Int).SetUint64(uint64(4248141415055360))

	for manager, capacity := range checkStorage {
		snRatio := snap.StorageData.calStorageRatio(capacity, number)
		spHash := manager.Hash()
		if sp,ok:=snap.SpData.PoolPledge[spHash];ok{
			nowCap:=new(big.Int).Set(sp.TotalCapacity)
			if nowCap.Cmp(capacity)<0 {
				sp.TotalCapacity=new(big.Int).Set(capacity)
				sp.SnRatio=snRatio.Mul(SnDefaultRatioDigit).BigInt()
				snap.SpData.accumulateSpPledgelHash(spHash, false)
			}
		}else{
			snap.SpData.PoolPledge[spHash] = &PoolPledge{
				...
				Number:         big.NewInt(initStorageManagerNumber - 1),
				TotalAmount:    common.Big0,
				TotalCapacity:  new(big.Int).Set(capacity),
				UsedCapacity:   common.Big0,
				PunishNumber:   common.Big0,
				SnRatio:        snRatio.Mul(SnDefaultRatioDigit).BigInt(),
				RevenueAddress: manager,
				ManagerAmount:  new(big.Int).SetInt64(0),
				Fee:            0,
				EntrustRate:    0,
				Status:         spStatusInactive,
